...
function GridOfBoxes() {
  ...
  // Detect mobile and run automatic animation
  useEffect(() => {
    const checkMobile = () => {
  ...
  // Automatic animation on mobile - multiple dynamic patterns
  useEffect(() => {
    if (!isMobile) {
      setHoveredBox(null);
      return;
    ...
    const patternDuration = 2000; // Switch pattern every 2 seconds

    // Keep patterns centered - use center area (3-7) for better visibility
    const center = gridSize / 2;
    const range = 2.5; // Stay within center ±2.5 cells

    const patterns = {
      // Spiral outward from center (smaller radius)
      spiral: (t: number) => {
        const angle = t * 2.5;
        const radius = (t % 1) * range;
        const x = Math.floor(center + Math.cos(angle) * radius);
        const z = Math.floor(center + Math.sin(angle) * radius);
        return [x, z];
      },

      // Random walk with momentum (constrained to center)
      randomWalk: (() => {
        let currentX = center, currentZ = center;
        let targetX = center, targetZ = center;
        let lastChange = 0;
        return (t: number) => {
          if (t - lastChange > 0.25) {
            targetX = center + (Math.random() - 0.5) * range * 2;
            targetZ = center + (Math.random() - 0.5) * range * 2;
            lastChange = t;
          }
          currentX += (targetX - currentX) * 0.15;
          currentZ += (targetZ - currentZ) * 0.15;
          return [Math.floor(currentX), Math.floor(currentZ)];
        };
      })(),
      ...
      circle: (t: number) => {
        const angle = t * 1.5;
        const x = Math.floor(center + Math.cos(angle) * range);
        const z = Math.floor(center + Math.sin(angle) * range);
        return [x, z];
      },

      // Bouncing in center area
      bounce: (t: number) => {
        const x = Math.floor(center + Math.sin(t * 1.8) * range);
        const bounceHeight = Math.abs(Math.sin(t * 3));
        const z = Math.floor(center + (bounceHeight - 0.5) * range * 2);
        return [x, z];
      },

      // Figure 8 pattern (centered)
      figure8: (t: number) => {
        const x = Math.floor(center + Math.sin(t * 1.2) * range);
        const z = Math.floor(center + Math.sin(t * 2.4) * range * 0.8);
        return [x, z];
      },
      ...
      pulse: (() => {
        ...
        return (t: number) => {
          ...
          const pulsePhase = (t - lastPulse) * 2.5;
          const angle = pulsePhase * 5;
          const radius = pulsePhase * range * 0.8;
          const x = Math.floor(center + Math.cos(angle) * radius);
          const z = Math.floor(center + Math.sin(angle) * radius);
          return [x, z];
        };
    ...
    const animate = () => {
      ...
      if (now - patternStartTime > patternDuration) {
        patternIndex = (patternIndex + 1) % patternKeys.length;
        patternStartTime = now;
      }

      const currentPattern = patterns[patternKeys[patternIndex]];
      const [x, z] = currentPattern(time);

      setHoveredBox([
        Math.min(Math.max(x, 0), gridSize - 1),
        Math.min(Math.max(z, 0), gridSize - 1)
      ]);
  ...
  useEffect(() => {
    const updateCamera = () => {
      ...
      if (camera instanceof THREE.PerspectiveCamera) {
        // Wider FOV on mobile to show more of the grid
        camera.fov = isMobile ? 70 : 35;
